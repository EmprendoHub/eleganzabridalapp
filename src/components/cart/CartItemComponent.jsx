                ...
                productData?.map((item ) => (
                    <div key={item?._id} className='w-full bg-white p-4 flex sm:flex-col flex-row items-center gap-4'>
                        <div className='flex items-center gap-x-3 w-full '>
                            ...
                            <Image loader={myLoader} src={ item?.mainImageUrl } width={500} height={500} alt='product image' className='w-[50%] sm:w-full mdsm:w-2/3 h-30 object-cover'/>
                        </div>
                        {/* Model Value */}
                        ...
                        <div className='flex  items-center justify-end gap-x-3 border-[1px] border-slate-300 py-2 px-4 w-auto'>
                            ...
                            <div className='flex items-center text-lg  w-20 justify-between'>
                                <span onClick={() => dispatch(decreaseQuantity(item)) && toast.success(`${item.title} disminuyo en cantidad`, {position: toast.POSITION.TOP_CENTER,
                                className: "foo-bar",
                                theme: "dark",
                                transition: Bounce
                                ...
                                <span onClick={()=> dispatch(increaseQuantity(item)) && toast.success(`${item.title} incremento en cantidad`, {position: toast.POSITION.TOP_CENTER,
                                className: "foo-bar",
                                theme: "dark",
                                transition: Bounce
                            })} className='cursor-pointer'>
