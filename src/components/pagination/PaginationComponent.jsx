...
const PaginationControllerComponent = ({
  ...
  return (
    <div className='paginate-container flex mx-auto item text-center flex-row justify-center gap-x-5 sm:gap-x-2 items-center'>
          <button className="bg-pink-600 disabled:bg-slate-300 text-white p-2 rounded-full text-xl" onClick={()=>{
          ...
          <button className="bg-pink-600 disabled:bg-slate-300 text-white p-2 rounded-full text-xl" onClick={()=>{
            router.push(`${prevSearchParams}&page=${Number(page) - 1}&per_page=${per_page}`)
            }}
            disabled={!hasPrevPage}
          >
            <FiChevronLeft/>
          ...
          <button className="bg-pink-600 text-xl text-white p-2 rounded-full disabled:bg-slate-300" onClick={()=>{
            router.push(`${prevSearchParams}&page=${Number(page) + 1}&per_page=${per_page}`)
            }}
          ...
          <button className="bg-pink-600 disabled:bg-slate-300 text-white p-2  rounded-full text-xl" onClick={()=>{
            router.push(`${prevSearchParams}&page=${Math.ceil(totalProductCount / Number(per_page))}&per_page=${per_page}`)
            }}
            disabled={!hasNextPage}
          >
            <FiChevronsRight/>
