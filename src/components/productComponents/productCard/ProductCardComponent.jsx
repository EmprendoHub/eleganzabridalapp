      ...
      <div className="rounded-sm overflow-hidden">
        <Link href={`/product/${product._id}`} className="">
          <div className="relative">
            ...
            {product?.isNewest && (
              <>
                <span className="absolute top-2 right-2 font-medium text-[10px] py-1 px-3 rounded-full bg-white group-hover:bg-pink-700 group-hover:text-white duration-200">
                  {lang === "en"
                    ? "New Arrival"
                    : "" || lang === "es"
                    ? "Nueva llegada"
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.9 }}
                  className="bg-black absolute top-2 left-3 p-1.5 text-sm tracking-wide rounded-full text-slate-100 hover:bg-pink-600 hover:text-white duration-500"
