          ...
          <div className='w-[80%] sm:w-full mx-auto wrapper-class grid grid-cols-2 md:grid-cols-1 gap-5 bg-white p-4 rounded-lg'>
           <div className='w-full flex justify-center'>
            ...
            <div className="flex flex-col justify-center gap-y-10">
              ...
              >
                <p className="text-5xl md:text-3xl font-semibold">{ lang === "en" && product?.title ? product?.title : "" || lang === "es" && product?.titulo ? product?.titulo : ""}</p>
              ...
              className="text-gray-700 description-class">{ lang === "en" && product?.description ? ( product?.description ): ("Our dresses are more than garments; they are statements of style, elegance, and individuality. Make your unforgettable moments even more special with our stunning collection. Choose timeless elegance and modern charm to elevate your style for weddings, proms, and sweet sixteens.") || lang === "es" && product?.descripcion ? ( product?.descripcion ): "Nuestros vestidos son más que prendas; son declaraciones de estilo, elegancia e individualidad. Haz que tus momentos inolvidables sean aún más especiales con nuestra impresionante colección. Elija elegancia atemporal y encanto moderno para elevar su estilo para bodas, graduaciones y quinceañeras."}</motion.div>
              <motion.div 
                  initial={{y:50, opacity:0 }} 
                  whileInView={{y:0, opacity: 1 }} 
                  transition={{duration: 0.7}}
              ...
              >
                ...
                <span>
                  {singleprod.brand}: <span className="text-darkText">{product?.brand}</span>
                </span>
                <span>
                {singleprod.category}: <span className="text-darkText">{product?.category}</span>
                </span>
                <span>
                {singleprod.tags}: <span className="text-darkText">{product?.tags}</span>
                </span>
              </motion.div>
              <motion.div
                  initial={{y:50, opacity:0 }} 
          ...
          <div className="px-8 sm:px-4 my-8 w-[80%] lg:w-full mx-auto">
            ...
            <div className="grid xsm:grid-cols-1 md:grid-cols-2 grid-cols-4 gap-4 mt-2">
              {trendingProducts?.map((product) => (
                <ProductCardComponet key={product._id} lang={lang} product={product} />
              ))}
            </div>
          </div>
