    ...
    <div className="w-full bg-white p-4">
      ...
      <div className="border-b-[1px] border-b-slate-300 py-2">
        <div className="max-w-lg flex items-center justify-between">
          ...
          <p className="font-bold pr-4">{totalQty}</p>
        </div>
      </div>
      {userInfo ? (
        <button className=" bg-black text-slate-100 mt-4 py-3 px-6 hover:bg-pink-700 cursor-pointer duration-500">
          Request your Quote
        </button>
      ) : (
        <div>
          ...
          {!session && (
            <div>
              <div
                onClick={() => signIn()}
                className=" flex flex-row w-[200px] bg-black text-slate-100 mt-4 py-3 px-6 hover:bg-pink-700 duration-500 cursor-pointer"
              >
